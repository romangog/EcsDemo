...
{
    EcsWorld _world;
    EcsFilter<EnemyTag, RigidbodyComponent, SpeedMultiplierComponent>.Exclude<DeadTag, HitThrowbackTimer> _enemyRigidbodyesFilter;
    ...
    public void Run()
    {
        foreach (var enemyId in _enemyRigidbodyesFilter)
        {
            ...
            foreach (var playerId in _playerPositionFilter)
            {
                ...
                Vector2 movement = distanceToPlayer.normalized * Time.deltaTime * 3f * enemySpeedMod.Multiplier;
